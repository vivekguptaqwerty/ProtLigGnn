...
class PDBbindPairDataset(Dataset):
    def __init__(self, records: Sequence[ComplexRecord]):
        ...
        for record in tqdm(self.records, desc="Processing complexes"):
            ...
            elif args.model_type == "soft_routing":
        ...
        config = RoutingConfig(
            ...
            learnable_temperature=(args.attention_learnable_scale == "true"),
    ...
    elif args.model_type == "foundation_hybrid":
        ...
        bias_config = AttentionBiasConfig(
            ...
            learnable_scale=(args.attention_learnable_scale == "true"),
            initial_alpha=args.attention_initial_alpha,
            normalize_bias=(args.attention_normalize_bias == "true"),
            num_heads=4
        )
        ...
        prot_config = ProteinFoundationConfig(
            ...
            latent_dimension=args.hidden_dim,
            projection_dimension=args.hidden_dim,
        ...
        model = HybridMultimodalGNN(config=hybrid_config, base_hybrid=base_hybrid).to(device)
        
    elif args.model_type == "foundation_hybrid_only":
        from models.foundation.hybrid.config import HybridFoundationConfig
        from models.foundation.hybrid.model import HybridFoundationOnly
        ...
        model = HybridFoundationOnly(config=hybrid_config).to(device)
...
def run_epoch(model, loader, device, optimizer=None):
    ...
    for ligand_batch, protein_batch, labels, batch_pdb_ids in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            if is_train:
                # Capture gradient conflict metrics if soft routing is active
                if hasattr(model, "affinity_router_l"):
                    optimizer.zero_grad()
                    affinity_loss.backward(retain_graph=True)
                    # Check shared cross-attention parameter gradients
                    p_shared = model.base_model.cross_attention.ligand_to_protein_attn.in_proj_weight
                    grad_aff = p_shared.grad.clone() if p_shared.grad is not None else None
                    
                    optimizer.zero_grad()
                    contact_loss.backward(retain_graph=True)
                    grad_contact = p_shared.grad.clone() if p_shared.grad is not None else None
                    
                    if grad_aff is not None and grad_contact is not None:
                        from models.soft_routing.routing_metrics import compute_gradient_conflict
                        step_conflict = compute_gradient_conflict(grad_aff, grad_contact)
                        step_conflict_list.append(step_conflict)
...
def main():
    ...
    test_out = run_epoch(model, test_loader, device, optimizer=None, epoch=1, max_epochs=args.epochs)
