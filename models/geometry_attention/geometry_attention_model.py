...
from models.geometry_attention.attention_bias import bias_factory

class DistanceBiasedCrossAttention(nn.Module):
    """Bidirectional Cross-Attention layer with explicit spatial distance biases."""
    
    def __init__(self, hidden_dim: int, config: AttentionBiasConfig, dropout: float = 0.1, 
        ...
        # 1. Instantiate pluggable attention bias using config
        self.attention_bias = bias_factory(
        ...
        # 2. Learnable scale parameter in log-space (exp(log_alpha) > 0)
    ...
    def forward(self, ligand_x: torch.Tensor, protein_x: torch.Tensor, 
                ligand_pos: torch.Tensor, protein_pos: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ligand_seq = ligand_x.unsqueeze(0)  # [1, L, D]
        protein_seq = protein_x.unsqueeze(0)  # [1, P, D]
        
        # 1. Compute raw bias using pluggable attention bias
        raw_bias = self.attention_bias(ligand_pos, protein_pos)  # [num_heads, L, P]
        
        # 2. Normalize bias if configured to keep it bounded
        ...
        # 4. Attention calculation passing the bias as attn_mask
        ...
        # 5. Residuals & layer normalization
...
class ProtLigGNNGeometryAttention(nn.Module):
    """ProtLigGNN model upgraded with Distance-Biased Cross-Attention."""
    ...
    def __init__(self, ligand_dim: int, protein_dim: int, hidden_dim: int = 128, 
                 ...
                 config: AttentionBiasConfig = None) -> None:
        super().__init__()
        self.no_crossgraph = no_crossgraph
        self.use_attention = use_attention
        self.hidden_dim = hidden_dim
        self.pooling = pooling
        self.config = config if config is not None else AttentionBiasConfig()
        if pooling not in {"mean", "max", "mean_max"}:
            raise ValueError(f"Unsupported pooling mode: {pooling}")
        
        # 1. Coordinate-free encoders identical to Baseline v1.1
        ...
        # 2. Distance-Biased Cross-Attention
        ...
        # 3. Regressor head. `mean_max` keeps both smooth and extremal pocket signals.
        regressor_input_dim = hidden_dim * 8 if pooling == "mean_max" else hidden_dim * 4
    ...
    def forward(self, ligand_batch: Batch, protein_batch: Batch) -> torch.Tensor:
        # 1. Encode graphs via baseline coordinate-free encoders
        ...
        # 2. Dynamic batch looping applying distance bias
        ligand_chunks = []
        protein_chunks = []
